	...
	if body.is_in_group("player"):
		...
		if obj.is_in_group("items") and obj.has_meta("food_id") and drive_thru_menu.food_id != -1:
			...
			if obj is PizzaBox:
				...
				GlobalSignal.leave_drive_thru.emit(drive_thru_menu.car)
...
func can_interact(_player: Player) -> bool:
	if in_range and drive_thru_menu.visible and is_store_open:
		_player.inputs_ui.update_actions.emit(_player.inputs_ui.InputAction.Interact, _player.has_held_object())
	return in_range
	
func interact(_player: Player) -> void:
	if not has_order and drive_thru_menu.visible and is_store_open:
